...
public class Matrix4 {
	...
	public void ortho(float left, float right, float bottom, float top,
		...
		float deltaY = top - bottom;
		float deltaZ = farZ - nearZ;
		float[] orthoMat = loadIdentityMatrix();
	...
	public void matrixMultiply(Matrix4 lhs, Matrix4 rhs){
		matrixMultiply(lhs.get(), rhs.get());
	}
	...
	private float[] loadIdentityMatrix() {
	...
	public void lookAt(Vector3 eye, Vector3 look, Vector3 up) {
		Vector3 n = eye.getNewVecSub(look);
		n.normalize();
		
		Vector3 u = up.getNewVecCross(n);
		u.normalize();
		
		Vector3 v = n.getNewVecCross(u);
		v.normalize();
		
		
		final float values[] = {u.x, u.y, u.z, 0,
...
//	    return c * Translate( -eye );
	...
	public void printMatrix(String tag) {
		int i = 0;
			Log.e(tag, mMatrix[i] + " x " + mMatrix[i++] + " x " +mMatrix[i++] + " x " +mMatrix[i++]);
			Log.e(tag, mMatrix[i] + " x " + mMatrix[i++] + " x " +mMatrix[i++] + " x " +mMatrix[i++]);
			Log.e(tag, mMatrix[i] + " x " + mMatrix[i++] + " x " +mMatrix[i++] + " x " +mMatrix[i++]);
			Log.e(tag, mMatrix[i] + " x " + mMatrix[i++] + " x " +mMatrix[i++] + " x " +mMatrix[i++]);
	}
